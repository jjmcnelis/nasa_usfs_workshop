...
def get_null_summary(xrdataset):
    ...
    percent = {}
    typebool = {"in situ": False, "airborne": False, "spaceborne": False}
    for name, dataset in xrdataset.items():
        t = dataset.attrs["type"]
        dsnull = dataset.isnull().sel(stat="Mean", drop=True)
...
def get_options(layer):
    ...
    options = ["None", "source", "type", "units", "soil_zone"]
    
    xrds = layer.xr
    tbool, tperc = get_null_summary(xrds)
    opt_lists = {opt:[] for opt in options}
    for dataset in tperc.keys():
        xrda = xrds[dataset].sel(stat="Mean", drop=True)
        for opt in groupby_options:
            if opt=="None":
                opt_lists[opt].append(dataset)
            else:
                opt_lists[opt].append(xrda.attrs[opt])
    opt_lists = {g:list(set(o))for g,o in opt_lists.items()}
    return(opt_lists)


def radio_checkbox(optdict):
    """Radio button for displaying checkbox filters."""
    
    groupby = RadioButtons(
        options=optdict.keys(),
        value="None",
        orientation="horizontal")
    
    chkbxdict = {grp: [Checkbox(
        description=str(a), 
        value=True, 
        indent=False,
        layout=Layout(width='auto')
    ) for a in opt] for grp, opt in optdict.items()}
    
    #chkbxout = Output(layout={
    #    "overflow": "scroll", 
    #    "width": "30%",
    #    "height": "200px",
    ...
    def update(change={"new": "None"}):
        group = change["new"]
        chkbxout.children = chkbxdict[group]
    groupby.observe(update, names="value")
    
    update()
    return(VBox(
        [HTML("<b>Group: </b>"), groupby, chkbxout]))#,
...
def get_symbology(typebool):
    """ """
    style = {"fill_color": "#cbd5e8", "fill_opacity": 0.4}
    if typebool["in situ"]:
        style.update({
            "fill_color": "#ff7f00", 
            "fill_opacity": 0.8, 
